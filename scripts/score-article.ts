...
const PINKY_ARTICLE_TITLE = "Meet Pinky: The AI Lab Rat Who Helps a Madman Take Over the World";

const PINKY_ARTICLE_CONTENT = `NARF!
...
async function main() {
  console.log('🐀 Scoring Pinky Article...\n');
  ...
  try {
    const score = await analyzeArticle(PINKY_ARTICLE_TITLE, PINKY_ARTICLE_CONTENT, googleKey);
    
    console.log('📊 StepTen Score:', score.weightedScore.toFixed(1));
    console.log('📈 Rating:', score.rating);
    console.log('\n--- SCORE BREAKDOWN ---');
    Object.entries(score.scores).forEach(([key, val]) => {
      console.log(`${key}: ${val.score}/100 - ${val.feedback}`);
    });
    
    console.log('\n--- IMPROVEMENTS ---');
    score.prioritizedImprovements.forEach((imp, i) => {
      console.log(`${i + 1}. [${imp.impact}] ${imp.action}`);
    });
    
    // Save
    const outputDir = path.join(process.cwd(), 'output');
    if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir);
    fs.writeFileSync(path.join(outputDir, 'pinky-score.json'), JSON.stringify(score, null, 2));
    console.log('\n💾 Score saved to output/pinky-score.json');
